...
export class PublicationsController {
  ...
  async createBook(@Res() response, @Body() publication: Publication) {
    const newPublication = await this.publicationService.createPublication(publication);
  ...
  @Put('/:id')
  async updateById(@Res() response, @Body() publication: Publication, @Param('id') id){
    const updateResult = await this.publicationService.updatePublication(id, publication);
